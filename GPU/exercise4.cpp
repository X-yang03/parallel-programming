...
#define tileY 4 
#define tileX 8     //改变这两个参数，测试结果并分析
  ...
  auto e = q.submit([&](sycl::handler &h) {
      h.parallel_for<class k_name_t>(
          sycl::nd_range<2>(global_ndrange, local_ndrange), [=](sycl::nd_item<2> index) {
              ...
              float subB[tileX][2] = { 0.0f };//增加为2维，增大一倍寄存器，可以存两个向量
              // core computation
              for (int k = 0; k < N; k++) {
                  // read data to register
                  for (int m = 0; m < tileY; m += 2) {
                      subA[m][0] = A[(row + m) ∗ N + k];
                      subA[m][1] = A[(row + m + 1) ∗ N + k]; //一次读两组A中的行向量
                  }
                  for (int p = 0; p < tileX; p += 2) {
                      subB[p][0] = B[k ∗ N + p + col];
                      subB[p][1] = B[k ∗ N + p + col + 1];   //一次读两组B中的列向量
                  }
                  for (int m = 0; m < tileY; m += 2) {
                      for (int p = 0; p < tileX; p += 2) {
                          sum[m][p] += subA[m][0] ∗ subB[p][0];
                          sum[m + 1][p + 1] += subA[m][1] ∗ subB[p][1];   //一次性完成两步
                      }
                  }
              } //end of K
